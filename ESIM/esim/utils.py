...
def sort_by_seq_lens(batch, sequences_lengths, descending=True):
    ...
    # print(sequences_lengths.device, sorting_index.device)
    ...
    # Ensure lengths are long dtype
    lengths = sequences_lengths.to(dtype=torch.long)

    # Sort by lengths (indices live on the same device as 'lengths')
    sorted_seq_lens, sorting_index = lengths.sort(0, descending=descending)

    # Make sure we index 'batch' with an index on the same device as 'batch'
    if sorting_index.device != batch.device:
        sorting_index = sorting_index.to(batch.device)

    sorted_batch = batch.index_select(0, sorting_index)

    # Build restoration_index on the same device as sorting_index
    _, reverse_mapping = sorting_index.sort(0, descending=False)
    idx_range = torch.arange(0, lengths.size(0),
                             device=reverse_mapping.device,
                             dtype=torch.long)
    restoration_index = idx_range.index_select(0, reverse_mapping)
